 ...
 * @author FryenX
 */
public class ManageUnits extends javax.swing.JFrame {

    /**
     * Creates new form ManageUnits
     */
    public ManageUnits() {
        initComponents();
        customizeTable();
        ...
        fetchUnits(searchQuery, pageNumber);
        Icon();
    }
    ...
    private ManageProducts manageProducts;  // Reference to ManageProducts

    public ManageUnits(ManageProducts manageProducts) {
    ...
    private void initComponents() {
        ...
        btnAddUnits.setBackground(new java.awt.Color(0, 153, 255));
        btnAddUnits.setFont(new java.awt.Font("Poppins", 0, 14)); // NOI18N
        btnAddUnits.setForeground(new java.awt.Color(255, 255, 255));
        btnAddUnits.setText("Add Data");
        btnAddUnits.setBorder(null);
        ...
        tableUnits.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        jScrollPane1.setViewportView(tableUnits);
        if (tableUnits.getColumnModel().getColumnCount() > 0) {
            tableUnits.getColumnModel().getColumn(0).setMinWidth(40);
            tableUnits.getColumnModel().getColumn(0).setPreferredWidth(40);
            tableUnits.getColumnModel().getColumn(0).setMaxWidth(40);
            tableUnits.getColumnModel().getColumn(2).setMinWidth(0);
            tableUnits.getColumnModel().getColumn(2).setPreferredWidth(0);
            tableUnits.getColumnModel().getColumn(2).setMaxWidth(0);
        }
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                        ...
                        .addGroup(layout.createSequentialGroup()
                            .addComponent(btnAddUnits, javax.swing.GroupLayout.PREFERRED_SIZE, 101, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGap(362, 362, 362)
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(btnAddUnits, javax.swing.GroupLayout.PREFERRED_SIZE, 47, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    public void btnAddUnitsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddUnitsActionPerformed
        JDialog addUnitDialog = new JDialog(this, "Add New Unit", true);
        addUnitDialog.setSize(300, 150);
        addUnitDialog.setLayout(new BorderLayout());
        ...
        JTextField txtUnitName = new JTextField(20); // Set preferred width for the input field

        formPanel.add(lblUnitName);
        formPanel.add(txtUnitName);
        ...
        addUnitDialog.add(inputPanel, BorderLayout.CENTER);
        addUnitDialog.add(buttonPanel, BorderLayout.SOUTH);

        // Handle Save button
        btnSave.addActionListener((ActionEvent e) -> {
            String unitName = txtUnitName.getText().trim();
            if (unitName.isEmpty()) {
                JOptionPane.showMessageDialog(addUnitDialog, "Unit name cannot be empty.", "Validation Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            try (PreparedStatement ps = conn.prepareStatement("INSERT INTO units (name) VALUES (?)")) {
                ps.setString(1, unitName);
                ps.executeUpdate();
                JOptionPane.showMessageDialog(addUnitDialog, "Unit added successfully!");
                addUnitDialog.dispose(); // Close the dialog

                if (manageProducts != null) {
                    manageProducts.productFetchUnits();  // Refresh the combo box in ManageProducts
                } else {
                    String searchQuery = txtSearch.getText().trim();
                    int pageNumber = 0; // Set the current page (0 for the first page)
                    fetchUnits(searchQuery, pageNumber);
                ...
                JOptionPane.showMessageDialog(addUnitDialog, "Error adding unit: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        // Handle Cancel button
        btnCancel.addActionListener(e -> addUnitDialog.dispose());

        addUnitDialog.setLocationRelativeTo(this); // Center dialog
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        ...
        DefaultTableModel model = (DefaultTableModel) tableUnits.getModel();
        if (selectedRow != -1) {
            int id = getSelectedUnitId();  // ID from the hidden column

            String unitName = (String) model.getValueAt(selectedRow, 1);  // Name from the second column

            // Retrieve the current search query
            String searchQuery = txtSearch.getText().trim();  // Get search query from the text field

            editUnit(id, unitName, searchQuery);  // Pass both ID, Name, and search query
        } else {
            JOptionPane.showMessageDialog(this, "Please select a unit first.", "No Selection", JOptionPane.WARNING_MESSAGE);
        }// TODO add your handling code here:
    }//GEN-LAST:event_btnEditActionPerformed

    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        int id = getSelectedUnitId();
        if (id != -1) {
            String searchQuery = txtSearch.getText().trim();
            deleteUnit(id, searchQuery); // Your edit method
        }// TODO add your handling code here:
    ...
    private void btnPrevActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPrevActionPerformed
        if (currentPage > 0) {
            currentPage--;
            fetchUnits(txtSearch.getText().trim(), currentPage);
        }// TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ManageUnits.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ManageUnits().setVisible(true);
            }
        });
    }

    private void fetchUnits(String searchQuery, int pageNumber) {
        int offset = pageNumber * pageSize; // Offset for pagination

        String sql = "SELECT id, name FROM units WHERE name LIKE ? LIMIT ? OFFSET ?";
    ...
    private void calculateTotalPages(String searchQuery) {
        String countSql = "SELECT COUNT(*) FROM units WHERE name LIKE ?";
    ...
    private int getSelectedUnitId() {
        ...
        if (model.getColumnCount() > 2) {
            ...
            Object unitIdObject = model.getValueAt(selectedRow, 2);
            if (unitIdObject instanceof Integer) {
                return (int) unitIdObject; // Return the category ID
            } else {
                JOptionPane.showMessageDialog(this, "Invalid unit ID format.", "Error", JOptionPane.ERROR_MESSAGE);
                return -1;
            }
        } else {
    ...
    private void deleteUnit(int id, String searchQuery) {
        int confirm = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete this unit?", "Confirm Delete", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
        if (confirm == JOptionPane.YES_OPTION) {
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM units WHERE id = ?")) {
                ps.setInt(1, id);
                ps.executeUpdate();
                JOptionPane.showMessageDialog(this, "Units deleted successfully!");
                int pageNumber = 0; // Set the current page (0 for the first page)
                fetchUnits(searchQuery, pageNumber);
                updatePrevNextButtons();
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(this, "Error deleting units: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }

    private void editUnit(int id, String unitName, String searchQuery) {
        // Create a JPanel for the custom dialog
        JPanel panel = new JPanel(new GridLayout(2, 1));
        JLabel lblUnitName = new JLabel("Edit Units Name:");
        JTextField txtUnitName = new JTextField(unitName);
        panel.add(lblUnitName);
        panel.add(txtUnitName);

        // Show the custom dialog without any icon
        int option = JOptionPane.showConfirmDialog(this, panel, "Edit Units", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

        if (option == JOptionPane.OK_OPTION) {
            String newUnitName = txtUnitName.getText().trim();
            if (!newUnitName.isEmpty()) {
                try (PreparedStatement ps = conn.prepareStatement("UPDATE units SET name = ? WHERE id = ?")) {
                    ps.setString(1, newUnitName);
                    ps.setInt(2, id);
                    ps.executeUpdate();
                    JOptionPane.showMessageDialog(this, "Unit updated successfully!");
                    int pageNumber = 0; // Set the current page (0 for the first page)
                    fetchUnits(searchQuery, pageNumber);  // Refresh table
                    updatePrevNextButtons();
                } catch (SQLException ex) {
                    JOptionPane.showMessageDialog(this, "Error updating unit: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Unit name cannot be empty.", "Validation Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void customizeTable() {
        ...
        tableUnits.setFont(tableFont);  // Apply to the table cells

        // Set font for the table's header
        JTableHeader tableHeader = tableUnits.getTableHeader();
        ...
        tableUnits.setRowHeight(30);  // Set a larger row height if needed
    }
